...
function registerTFGWoodenRecipes(event) {
	...
	// Apricorn and saccharine wood. Cobblemon's own recipes are already gone -
	// cobblemon/recipes.js wipes the mod wholesale - so this is the only source of
	// worked apricorn and saccharine wood, and it is the same lumber chain every
	// other wood in the pack uses. Cobblemon has no support beams, chests, barrels
	// or the rest of TFC's wooden furniture, hence the run of nulls.
	global.COBBLEMON_WOOD.forEach(wood => {
		TFGWoodBuilder(
			...
			null,                  // jarbnet
			null,                  // big barrel
			null,                  // stomping barrel
			null,                  // barrel press
			null,                  // wine shelf
			wood.sign,
			wood.hanging_sign,
			null,                  // crafting station
			null,                  // workbench as material
			null                   // rope ladder
		);

		// GT recycling. The shared index keys off TFC's {mod}:wood/planks/{type}
		// id shape and Cobblemon's ids are flat (cobblemon:apricorn_planks), so the
		// material info is registered by hand here, at the index's own ratios:
		// 2 lumber per plank block, and each worked form costing what it is built from.
		const material = wood.isHardwood ? GTMaterials.get('hardwood') : GTMaterials.Wood;
		[
			[wood.plank, 2],
			[wood.door, 3 / 2],
			[wood.trapdoor, 1],
			...
			[wood.slab, 1],
			[wood.stair, 3 / 2],
			[wood.pressure_plate, 2],
			[wood.button, 1 / 4],
			[wood.sign, 1]
		].forEach(([item, amount]) => TFGHelpers.registerMaterialInfo(item, [material, amount]));
	});
